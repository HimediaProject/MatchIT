...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchMeta = async () => {
      try {
        const [cls, ers, stats] = await Promise.all([
          ...
          metaApi.getStats().catch((e) => {
            console.error(e); return { job_posts: 0, bootcamps: 0 }
          }),
        ])
        ...
        setJobCount(stats.job_posts || 0)
        setBootcampCount(stats.bootcamps || 0)
  ...
  // 오늘 날짜를 YYYY-MM-DD 형식으로 포맷하는 헬퍼 함수
  ...
  // DB에서 인기순 채용공고와 부트캠프공고 가져오기
  useEffect(() => {
    const fetchRecommendations = async () => {
      try {
        setLoadingRecommendations(true)
        
        // 채용공고 2개 가져오기 (조회수순 + 랜덤)
        const jobsResponse = await searchApi.searchWithFilters({
          source: '채용',
          limit: 2,
          sort: 'views' as SortOption,
          randomOrder: true,
        })
        
        // 부트캠프공고 2개 가져오기 (조회수순 + 랜덤)
  ...
  // DB에서 실시간 공고 미리보기 가져오기 (채용 2개 + 부트캠프 1개)
  useEffect(() => {
    const fetchPreviewItems = async () => {
      try {
        ...
        // 채용공고를 PreviewItem 형태로 변환
        const jobItems: PreviewItem[] = jobs.slice(0, 2).map((job: any) => {
          const date = job.created_at ? new Date(job.created_at).toISOString().split('T')[0] : '';
          const tags = [
            job.job_category,
            job.location,
            job.experience_requirement,
            ...(job.skills ? job.skills.slice(0, 2) : [])
          ].filter(Boolean);
          return {
            title: job.title || '채용공고',
        ...
        // 부트캠프공고를 PreviewItem 형태로 변환
        const bootcampItems: PreviewItem[] = bootcamps.slice(0, 1).map((bootcamp: any) => {
          const date = bootcamp.created_at ? new Date(bootcamp.created_at).toISOString().split('T')[0] : '';
          const tags = [
            bootcamp.location,
            bootcamp.cost_support_type,
            bootcamp.online_offline
          ].filter(Boolean);
          return {
            title: bootcamp.title || '부트캠프',
            company: bootcamp.institute_name || '',
            tags,
            type: '부트캠프',
            url: bootcamp.detail_url || '#',
            date,
        ...
        // 채용 2개 + 부트캠프 1개 합치기
        setPreviewItems([...jobItems, ...bootcampItems])
      } catch (error) {
        console.error('Failed to fetch preview items:', error)
        // 에러 발생 시 빈 배열 설정
        setPreviewItems([])
      } finally {
        ...
        <div className="relative mx-auto grid max-w-6xl grid-cols-1 items-center gap-10 px-4 py-16 md:px-6 lg:grid-cols-2 lg:py-20">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 rounded-2xl border border-slate-100 bg-white/80 p-4 shadow-soft sm:grid-cols-4">
              <div>
                <p className="text-2xl font-bold text-slate-950">+{Math.floor(jobCount / 10) * 10}</p>
                <p className="text-xs text-slate-500">채용 공고</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-slate-950">+{Math.floor(bootcampCount / 10) * 10}</p>
          ...
          <div className="relative">
            ...
            <div className="relative rounded-3xl border border-slate-100 bg-white/90 p-6 shadow-soft">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold text-primary-700">실시간 공고</p>
                  <h3 className="text-xl font-bold text-slate-900">채용과 부트캠프 실시간 공고</h3>
                  <p className="text-sm text-slate-500">실시간으로 업데이트되는 맞춤 공고를 한눈에</p>
                </div>
              ...
              <div className="mt-6 space-y-3">
                {loadingPreview ? (
                  <>
                    <div className="flex items-start justify-between rounded-2xl border border-slate-100 bg-slate-50/60 p-4">
                      <div className="space-y-1">
                        ...
                        <div className="h-3 w-1/4 animate-pulse rounded bg-slate-200"></div>
                      </div>
                    </div>
                    <div className="flex items-start justify-between rounded-2xl border border-slate-100 bg-slate-50/60 p-4">
                      <div className="space-y-1">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="mt-2 flex items-center gap-1 overflow-hidden">
                          {item.tags.slice(0, 3).map((tag, index) => (
                            <span key={index} className="rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-700 whitespace-nowrap">
                              {tag}
                            </span>
                          ))}
                          {item.tags.length > 3 && <span className="text-xs font-semibold text-slate-700">...</span>}
                        </div>
                      </div>
                      <div className="flex h-full items-center">
                        <span className="whitespace-nowrap px-3 py-1 text-xs font-semibold text-gray-500">{item.date || getTodayDate()}</span>
                      </div>
                    </div>
                  ))
                )}
      ...
      <section className="border-t border-slate-100 bg-gradient-to-b from-white to-slate-50">
        <div className="mx-auto max-w-6xl px-4 py-14 md:px-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-sm font-semibold text-primary-700">맞춤 추천 준비 단계</p>
              <h2 className="text-2xl font-bold text-slate-950 sm:text-3xl">복잡한 검색 없이, 나에게 맞는 기회부터 바로 보기</h2>
              <p className="mt-2 text-sm text-slate-600">
                지금 입력하는 정보를 바탕으로 우선 추천을 진행하고, 이후에는 프로필과 연동해 더 정교한 맞춤 추천을 제공합니다.
              </p>
            </div>
            <button
              onClick={() => {
