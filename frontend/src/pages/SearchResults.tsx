...
const SearchResultsPage = () => {
  ...
  const source = query.get('source') as 'all' | 'jobs' | 'bootcamps' | null
  ...
  if (source && source !== 'all') filterInfo.push(`출처: ${source}`)
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12 md:px-6">
      <div className="mb-6">
        ...
        {filterInfo.length === 0 && <p className="text-sm text-slate-600">검색 조건을 입력해 주세요.</p>}
      </div>
        ...
        <section className="mb-8">
          <div className="flex items-center justify-between">
            ...
            {jobs.length > 5 && view !== 'jobs' && (
              <Link to={buildViewUrl('jobs')} className="text-sm font-semibold text-primary-700 hover:text-primary-800">
                더 보기
              </Link>
            )}
          </div>
            ...
            {displayedJobs.map((j, idx) => (
              <div key={idx} className="rounded-lg border p-4">
                ...
                <p className="mt-2 text-sm text-slate-700">
                  {j.main_tasks ? j.main_tasks.slice(0, 200) + (j.main_tasks.length > 200 ? '...' : '') : ''}
                </p>
              </div>
            ))}
        ...
        <section>
          <div className="flex items-center justify-between">
            ...
            {bootcamps.length > 5 && view !== 'bootcamps' && (
              <Link to={buildViewUrl('bootcamps')} className="text-sm font-semibold text-primary-700 hover:text-primary-800">
                더 보기
              </Link>
            )}
          </div>
          <div className="mt-4 space-y-3">
            {displayedBootcamps.map((b, idx) => (
              <div key={idx} className="rounded-lg border p-4">
                ...
                <p className="mt-2 text-sm text-slate-700">
                  {b.education_content ? b.education_content.slice(0, 200) + (b.education_content.length > 200 ? '...' : '') : ''}
                </p>
              </div>
            ))}
