...
실행 방법:
    python 8_1_import_preprocessed_to_db.py

필요 패키지:
    pip install psycopg2-binary sqlalchemy

전제 조건:
    - embedding_data_data.json 파일이 존재해야 함
...
def load_embedding_data(filepath: str = 'embedding_data.json') -> Dict:
    """임베딩 데이터 로드"""
...
def insert_job_posts(db_session, preprocessed_jobs: List[Dict], master_data: MasterDataManager):
    ...
    for idx, job_item in enumerate(preprocessed_jobs, 1):
        try:
            processed = job_item.get('processed', {})
            # Embedding 벡터 가져오기
            embedding_vector = job_item.get('combined_embedding')
            ...
            if existing:
                ...
                """), {
                    ...
                    "embeded": embedding_vector # 리스트 형태 그대로 전달 (pgvector가 처리)
                })
...
def insert_bootcamp_posts(db_session,
    ...
    for idx, bootcamp_item in enumerate(preprocessed_bootcamps, 1):
        try:
            processed = bootcamp_item.get('processed', {})
            # embedding 벡터 가져오기
            embedding_vector = bootcamp_item.get('combined_embedding')
...
def main():
    ...
    # 전처리된 데이터 로드
    try:
        data = load_embedding_data('embedding_data.json')
    except FileNotFoundError:
        print("❌ embedding_data.json 파일을 찾을 수 없습니다!")
        return
