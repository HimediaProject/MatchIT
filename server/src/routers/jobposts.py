...
def create_job_post(  
    job: JobPostCreate,  
    db: Session = Depends(get_db),  # get_db 의존성을 통해 SQLAlchemy 세션을 주입받음
):  
...
def get_job_posts( 
    ...
    keyword: Optional[str] = Query(None, description="검색 키워드 (제목, 회사명, 스킬 이름)"), 
    category_id: Optional[int] = Query(None, description="직무 카테고리 ID"),
    location: Optional[str] = Query(None, description="지역 필터"),  
    experience_requirement: Optional[str] = Query(None, description="경력 요건 필터 (신입/경력)"),
    ...
    offset = (page - 1) * size 
    jobs = (  
        query.order_by(JobPost.CreatedAt.desc()) 
        .offset(offset)  
        .limit(size) 
        .all()  
    )
...
def get_job_post_detail( 
    ...
    job_post.UpdatedAt = datetime.now(job_post.UpdatedAt.tzinfo) if job_post.UpdatedAt else datetime.datetime.now(timezone.utc)
    db.commit()
    db.refresh(job_post)
